...
#[doc = "Field `EVENTS_STARTED` writer - Confirms that the EPIN\\[n\\].PTR and EPIN\\[n\\].MAXCNT, or EPOUT\\[n\\].PTR and EPOUT\\[n\\].MAXCNT registers have been captured on all endpoints reported in the EPSTATUS register"]
pub type EVENTS_STARTED_W<'a, const O: u8> =
    crate::BitWriter<'a, EVENTS_STARTED_SPEC, O, EVENTS_STARTED_A>;
